...
app.post('/database/table/create', async function(req, res){
    ...
    databaseCon.connect(
        function(err){
            ...
            const sql = "CREATE TABLE organizations (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), username VARCHAR(255), password VARCHAR(255), userType VARCHAR(255), role VARCHAR(255), information TEXT, batchSize INT)";
...
app.get('/organizations/roles', async function(req, res){
    ...
    databaseCon.connect(
        function(err){
            ...
            console.log("Connected to database.");
            const sql = "SELECT id, username, role FROM organizations";
            databaseCon.query(sql, function (err, result) {
                if (err) {
                    console.log(err);
                    return;
